...
now = datetime.now()
filename = "tiptrack_data_" + now.strftime("%d_%m_%Y_%H_%M_%S.csv")
...
while True:
	...
	data = data.split()
	if len(data) != 13: # Startup messages
		continue
	cal = [int(k.split("=")[1]) for k in data[9:]]
	if(cal[1] < 3 or cal[3] < 3):	# Not calibrated
		print("System cal: " + str(cal[0]) + " gyro: " + str(cal[1]) + " accel: " + str(cal[2]) + " mag: " + str(cal[3]))
		# iterator = 0
		# loop = False
		continue
	quat = [float(data[i]) for i in range(1,9,2)]
	if sum(quat) == 0:	# No actual data yet
		continue
	if iterator < 500:
		print("waiting to compensate... ")
		iterator += 1
		continue
