...
def main():
	...
	while True:
		...
		counter += 1
		if(counter % 20 == 0):
			t0.set_offsets([[id,therms[id][0]] for id in ids])
			t1.set_offsets([[id,therms[id][1]] for id in ids])
			t2.set_offsets([[id,therms[id][2]] for id in ids])
			t3.set_offsets([[id,therms[id][3]] for id in ids])
			tmp.set_offsets([[id, sht31[id][0]] for id in ids])
			hmd.set_offsets([[id, sht31[id][1]] for id in ids])
